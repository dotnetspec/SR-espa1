...
update msg model =
    case ( msg, model ) of
                            ...
                            case user of
                                ...
                                Data.Users.Registered userInfo userState ->
                                    let
                                        ...
                                        newUser =
                                            Data.Users.removedDeletedRankingsFromUserJoined user Data.Rankings.empty

                                        _ =
                                            Debug.log "newUser: " newUser

                                        updatedsUsers =
                                            Data.Users.updatedUserInSet sUsers newUser
                                    in
        ...
        ( ClickedSelectedRanking uR, AppOps (Fetched sUsers sRankings _) user uiState txRec ) ->
            case uR.rankingtype of
                Data.Global.Owned ->
                    let
                        newDataKind =
                            Selected <| Data.Selected.created EverySet.empty sUsers user uR.rankingInfo Data.Selected.Owner

                        newDataState =
                            Fetched sUsers sRankings newDataKind
                    in
                    ( AppOps newDataState
                        user
                        (SR.Types.SelectedUI SR.Types.Owned)
                        txRec
                ...
                Data.Global.Member ->
                    let
                        newDataKind =
                            Selected <| Data.Selected.created EverySet.empty sUsers user uR.rankingInfo Data.Selected.Member
                    ...
                    ( AppOps newDataState
                        user
                        (SR.Types.SelectedUI SR.Types.Member)
                        txRec
                    , fetchedSingleRanking uR
                    )
                ...
                Data.Global.Other ->
                    let
                        newDataKind =
                            Selected <| Data.Selected.created EverySet.empty sUsers user uR.rankingInfo Data.Selected.Other
                    ...
                    ( AppOps newDataState
                        user
                        (SR.Types.SelectedUI SR.Types.Other)
                        txRec
                    , fetchedSingleRanking uR
                    )
        ...
        ( SentResultToWallet result, AppOps dataState user uiState txRec ) ->
            ...
            case result of
                ...
                Data.Selected.Undecided playerUP challengerUP ->
                    ( AppOps dataState
                        user
                        (SR.Types.SelectedUI SR.Types.ConfirmedResult)
                        emptyTxRecord
                    , sentryCmd
                    )
                        ...
                        Fetched sUsers sRankings dKind ->
                            case dKind of
                                Selected sSelected ->
                                    let
                                        newDataKind =
                                            Selected (Data.Selected.handleWon sSelected playerUP challengerUP sUsers)
                                ...
                                _ ->
                                    let
                                        _ =
                                            Debug.log "2.1 - dataState" dataState
                                    in
                                    ( model, Cmd.none )
                        ...
                        Fetched sUsers sRankings dKind ->
                            case dKind of
                                Selected sSelected ->
                                    let
                                        newDataKind =
                                            Selected (Data.Selected.handleLost sSelected playerUP challengerUP sUsers)
                                ...
                                _ ->
                                    let
                                        _ =
                                            Debug.log "3 - dataState" dataState
                                    in
                                    ( model, Cmd.none )
                ...
                Data.Selected.Undecided playerUP challengerUP ->
                    case dataState of
                        Fetched sUsers sRankings dKind ->
                            case dKind of
                                Selected sSelected ->
                                    let
                                        newDataKind =
                                            Selected (Data.Selected.handleUndecided sSelected playerUP challengerUP)
                                ...
                                _ ->
                                    let
                                        _ =
                                            Debug.log "4 - dataState in Undecided" dataState
                                    in
                                    ( model, Cmd.none )
                        ...
                        _ ->
                            let
                                _ =
                                    Debug.log "5 - dataState in undecided" dataState
                            in
                            ( model, Cmd.none )
        ...
        --             in
        --                 (newModel, Cmd.none)
        ( ClickedCreateNewLadder, AppOps (Fetched sUsers sRankings (Global g)) (Data.Users.Registered userInfo userState) uiState txRec ) ->
            let
                newg =
                    Data.Global.createdNewUR g (Data.Users.Registered userInfo userState)

                newDataState =
                    Fetched sUsers sRankings (Global newg)
        ...
        ( Cancel, AppOps (Fetched sUsers sRankings dKind) user uiState txRec ) ->
            let
                newDataKind =
                    Global (Data.Global.created sRankings sUsers user)
        ...
        ( LadderNameInputChg namefield, AppOps (Fetched sUsers sRankings (Global g)) user uiState txRec ) ->
            let
                newg =
                    Data.Global.updateRankingName g namefield
        ...
        ( LadderDescInputChg descfield, AppOps (Fetched sUsers sRankings (Global g)) user uiState txRec ) ->
            let
                newg =
                    Data.Global.updateRankingDesc g descfield
        ...
        ( UserNameInputChg updateField, AppOps dataState (Data.Users.Spectator userInfo userState) uiState txRec ) ->
            ( AppOps dataState
                (Data.Users.Spectator { userInfo | username = updateField } userState)
                uiState
                txRec
            , Cmd.none
            )

        ( UserNameInputChg updateField, AppOps dataState (Data.Users.Registered userInfo userState) uiState txRec ) ->
            ( AppOps dataState (Data.Users.Registered { userInfo | username = updateField } userState) uiState txRec, Cmd.none )

        ( UserPasswordInputChg updateField, AppOps dataState (Data.Users.Spectator userInfo userState) uiState txRec ) ->
        ...
        ( ClickedChallengeOpponent userAsUserPlayer opponentAsUserPlayer, AppOps (Fetched sUsers sRankings (Selected sSelected)) (Data.Users.Spectator userInfo userState) uiState txRec ) ->
            ( Failure "Spectator should'nt be able to challenge", Cmd.none )

        ( ClickedChallengeOpponent userAsUserPlayer opponentAsUserPlayer, AppOps (Fetched sUsers sRankings (Selected sSelected)) (Data.Users.Registered userInfo userState) uiState txRec ) ->
            let
                newDataKind =
                    Selected (Data.Selected.assignedChallengerUIDForBOTHPlayers sSelected userAsUserPlayer opponentAsUserPlayer)
        ...
        ( ClickedDeleteRanking, AppOps dataState user uiState txRec ) ->
            case dataState of
                Fetched sUsers sRankings dKind ->
                    case dKind of
                        Selected sSelected ->
                            case user of
                                ...
                                Data.Users.Registered userInfo userState ->
                                    let
                                        newsUsers =
                                            Data.Users.updatedUserInSet sUsers user
                                        ...
                                        --todo: replace with a real set of rankings
                                        removedRanking =
                                            Data.Rankings.removedById (Data.Selected.gotRankingId sSelected) Data.Rankings.empty

                                        newDataKind =
                                            Global (Data.Global.created removedRanking sUsers user)

                                        newDataState =
                                            Fetched newsUsers sRankings newDataKind

                                        _ =
                                            Debug.log "ranking should have been removed from rankings" removedRanking
                                    in
                                    ( AppOps
                                        newDataState
                                        user
                        ...
                        _ ->
                            let
                                _ =
                                    Debug.log "8 - dataState should be Fetched" dataState
                            in
                            ( model, Cmd.none )
                ...
                _ ->
                    let
                        _ =
                            Debug.log "9 - dataState" dataState
                    in
                    ( model, Cmd.none )
        ...
        --         _ ->
        --             (model, Cmd.none)
        ( ClickedConfirmCreateNewRanking _, AppOps _ (Data.Users.Spectator userInfo userState) _ _ ) ->
            ( Failure "Cannot create a ladder as a spectator", Cmd.none )

        ( ClickedConfirmCreateNewRanking ranking, AppOps (Fetched sUsers sRankings dKind) (Data.Users.Registered userInfo userState) (SR.Types.GlobalUI SR.Types.NewLadderCreate) txRec ) ->
            case userInfo.walletState of
                Data.Users.Credited _ ->
                    let
                        ...
                        newesRankings =
                            Data.Rankings.addRanking ranking sRankings

                        newg =
                            Data.Global.created newesRankings sUsers (Data.Users.Registered userInfo userState)

                        newDataState =
                            Fetched sUsers newesRankings (Global newg)
                    in
                    ( AppOps newDataState
                        (Data.Users.Registered userInfo
                            --Data.Users.WalletWaitingForTransactionReceipt
                            Data.Users.Subscribe
                        )
                        (SR.Types.GlobalUI SR.Types.NewLadderConfirmed)
                        { txRec | txSentry = newSentry }
                    , sentryCmd
                    )
...
updateWithReceivedUsers model response =
    case ( model, response ) of
        -- AllEmpty, so fill the User set
        ( AppOps AllEmpty user uiState txRec, Ok lusers ) ->
            let
                filteredFUserList =
                    Utils.MyUtils.removeNothingFromList (Maybe.withDefault [] lusers)

                -- need to convert from FRanking to Ranking (id_ needs to be a String)
                lFromFToUser =
                    List.map Data.Users.convertFUserToUser filteredFUserList

                --_ = Debug.log "lFromFToUser : " lFromFToUser
                sUsers =
                    Data.Users.asUsers (EverySet.fromList lFromFToUser)

                newDataState =
                    Fetched sUsers Data.Rankings.empty (Global Data.Global.empty)
            in
            AppOps newDataState user uiState txRec

        ( AppOps (Fetched sUsers sRankings (Global _)) user uiState txRec, Ok lusers ) ->
            if Data.Rankings.isEmpty sRankings then
                -- just fill the User set
                let
                    filteredFUserList =
                        Utils.MyUtils.removeNothingFromList (Maybe.withDefault [] lusers)
                    ...
                    newDataState =
                        Fetched sUsers sRankings (Global Data.Global.empty)
                in
                AppOps newDataState user uiState txRec

            else
                --if sRankings isn't empty we can populate Global now
                let
                    filteredFUserList =
                        Utils.MyUtils.removeNothingFromList (Maybe.withDefault [] lusers)
                    ...
                    newDataKind =
                        Global (Data.Global.created sRankings newsUsers user)
...
updateWithReceivedPlayersByRankingId :
    ...
    ->
        Result
            (GQLHttp.Error (Maybe (List (Maybe Data.Players.FPlayer))))
            (Maybe (List (Maybe Data.Players.FPlayer)))
    -> Model
updateWithReceivedPlayersByRankingId model response =
    case ( model, response ) of
        ( AppOps (Fetched sUsers sRankings (Selected s)) user uiState txRec, Ok lplayers ) ->
            let
                -- filterMap?:
                filteredFPlayerList =
                ...
                lFromFToPlayer =
                    List.map Data.Players.convertPlayerFromFPlayer filteredFPlayerList
                ...
                newsSelected =
                    Data.Selected.created (EverySet.fromList lFromFToPlayer) sUsers user (Data.Selected.gotRanking s) (Data.Selected.gotStatus s)
        ...
        ( AppOps (Fetched sUsers sRankings (Selected s)) user uiState txRec, Err _ ) ->
            ...
            let
                ...
                newsSelected =
                    Data.Selected.created (EverySet.fromList Data.Players.dummyPlayers) sUsers user (Data.Selected.gotRanking s) (Data.Selected.gotStatus s)
...
updateWithReceivedRankings model response =
    case ( model, response ) of
        -- AllEmpty, so fill the Ranking set
        ( AppOps AllEmpty user uiState txRec, Ok lrankings ) ->
            let
                ...
                -- need to convert from FRanking to Ranking (id_ needs to be a String)
                lFromFToRanking =
                    List.map Data.Rankings.convertFRankingToRanking filteredFRankingList

                --_ = Debug.log "lFromFToRanking : " lFromFToRanking
                --newsRankings = Data.Rankings.asRankings (EverySet.fromList lFromFToRanking)
                -- nodb:
                newsRankings =
                    Data.Rankings.asRankings (EverySet.fromList Data.Rankings.testRankings)

                newDataState =
                    Fetched Data.Users.empty newsRankings (Global Data.Global.empty)
            in
            AppOps newDataState user uiState txRec

        ( AppOps (Fetched sUsers sRankings (Global _)) user uiState txRec, Ok lrankings ) ->
            if Data.Users.isEmpty sUsers then
                -- just fill the Ranking set
                let
                    filteredFRankingList =
                        Utils.MyUtils.removeNothingFromList (Maybe.withDefault [] lrankings)
                ...
                --if sUsers isn't empty we can populate Global now
                let
                    filteredFRankingList =
                        Utils.MyUtils.removeNothingFromList (Maybe.withDefault [] lrankings)
                    ...
                    newDataKind =
                        Global (Data.Global.created newsRankings sUsers user)

                    newDataState =
                        Fetched sUsers newsRankings newDataKind
                in
                AppOps newDataState user uiState txRec
        ...
        ( AppOps AllEmpty user uiState txRec, Err _ ) ->
            ...
            let
                ...
                newDataKind =
                    Global (Data.Global.created newsRankings Data.Users.empty user)

                newDataState =
                    Fetched Data.Users.empty newsRankings newDataKind
            in
            AppOps newDataState user uiState txRec
        ...
        ( AppOps (Fetched sUsers sRankings dKind) user uiState txRec, Err _ ) ->
            ...
            let
                ...
                newDataKind =
                    Global (Data.Global.created newsRankings Data.Users.empty Data.Users.emptyUser)
...
updateWithReceivedPlayers :
    Model
    -> Result (GQLHttp.Error (Maybe (List (Maybe Data.Players.FPlayer)))) (Maybe (List (Maybe Data.Players.FPlayer)))
    -> Data.Selected.MemberStatus
    -> Model
    ...
    case ( model, response ) of
        ...
        ( AppOps (Fetched sUsers sRankings (Selected s)) user uiState txRec, Ok lplayers ) ->
            let
                ...
                _ =
                    Debug.log "players" lFromFToPlayer

                newDataKind =
                    Selected <| Data.Selected.created (EverySet.fromList lFromFToPlayer) sUsers user (Data.Selected.gotRanking s) userOwnerStatus
    ...
    case ( model, response ) of
        ...
        ( AppOps (Fetched sUsers sRankings (Global sGlobal)) user uiState txRec, Ok franking ) ->
            let
                ...
                fromFToRanking =
                    Data.Rankings.Ranking "" True "" Nothing ""
                ...
                newDataState =
                    Fetched sUsers sRankings (Selected Data.Selected.empty)
            in
            AppOps newDataState user uiState txRec
...
createNewRankingResponse model response =
    case ( model, response ) of
        ( AppOps dataState (Data.Users.Spectator userInfo userState) uiState txRec, Ok createNewRankingResult ) ->
            Failure "Spectator can't create a ranking!"

        ( AppOps (Fetched sUsers sRankings dKind) (Data.Users.Registered userInfo userState) uiState txRec, Ok createNewRankingResult ) ->
            let
                newRanking =
                    Data.Rankings.convertFRankingToRanking createNewRankingResult

                firstPlayer =
                    Data.Players.IndividualPlayer (Data.Players.PlayerInfo newRanking.id_ userInfo.id 1)
                        Data.Players.Available

                firstUserPlayer =
                    { player = firstPlayer, user = Data.Users.Registered userInfo userState }

                newDataKind =
                    Selected <|
                        Data.Selected.created (EverySet.fromList [ firstPlayer ])
                            sUsers
                            (Data.Users.Registered userInfo userState)
                            newRanking
                            Data.Selected.Owner
...
updateAppInfoUserDesc descfield userInfo =
    let
        existingExtraUserInfo =
            userInfo.extrauserinfo

        newExtraInfo =
            { existingExtraUserInfo | description = descfield }

        newUserInfo =
            { userInfo | extrauserinfo = newExtraInfo }
    in
    newUserInfo
...
updateAppInfoUserEmail emailfield userInfo =
    let
        ...
        newExtraInfo =
            { existingExtraUserInfo | email = emailfield }
...
updateUserMobile mobilefield userInfo =
    let
        ...
        newExtraInfo =
            { existingExtraUserInfo | mobile = mobilefield }
        ...
        AppOps dataState user uiState txRec ->
            case dataState of
                    ...
                    case dKind of
                        Selected s ->
                            let
                                ...
                                newDataKind =
                                    Selected <|
                                        Data.Selected.created EverySet.empty
                                            sUsers
                                            user
                                            (Data.Selected.gotRanking s)
                                            (Data.Selected.gotStatus s)
                        ...
                        _ ->
                            let
                                _ =
                                    Debug.log "updateSelectedRankingPlayerList - dataState should be Selected (Fetched)" dataState
                            in
                            model
                ...
                _ ->
                    let
                        _ =
                            Debug.log "updateSelectedRankingPlayerList - dataState" dataState
                    in
                    model
...
configureThenAddPlayerRankingBtns s uplayer =
    ...
    let
        challorAvail =
            Data.Selected.challorAvail s uplayer
    in
        ...
        ( Data.Users.Registered userInfo userState, Data.Players.IndividualPlayer playerInfo playerStatus ) ->
            if not (Data.Selected.isChallenged uplayer) then
                if Data.Selected.isRegisteredPlayerCurrentUser (Data.Selected.gotUP s).user uplayer then
                    ...
                    let
                        _ =
                            Debug.log "got userUP " (Data.Selected.gotUP s)

                        _ =
                            Debug.log "got oppUP " (Data.Selected.gotOpponentAsUP s)
                    in
                    Element.column Grid.simple <|
            ...
            else if Data.Selected.isRegisteredPlayerCurrentUser (Data.Selected.gotUP s).user uplayer then
