...
testWalletSentry : Eth.Sentry.Wallet.WalletSentry
testWalletSentry =
          ...
          test "happy path: successful creation of a new ladder" <|
            \() ->
                start
                    ...
                    |> ProgramTest.update (Main.WalletStatus testWalletSentry)
                    --|> ProgramTest.update (Main.PollBlock (Ok 123))
                    |> ProgramTest.update (Main.GotGlobalRankingsJson globalRankingsJson)
                    |> expectViewHas
                        [ text "SportRank"
